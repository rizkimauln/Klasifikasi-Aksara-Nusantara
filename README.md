...
Folder **`Dataset`** sengaja **tidak disertakan** di repositori ini karena ukurannya yang besar. Anda diwajibkan untuk mengunduhnya secara manual melalui link berikut:
...
1. Unduh *file* berformat zip dari link di atas.
...
Clone repositori ini ke komputer Anda lalu masuk ke direktori proyek:
```bash
git clone <URL_REPOSITORY_ANDA>
cd nama-folder-repo
```
*(Ingat: Jangan lupa menempatkan folder `Dataset` hasil unduhan Anda ke dalam folder proyek ini).*
...
npm run dev
```
Buka *browser* Anda dan kunjungi URL yang diberikan (umumnya `http://localhost:5173`). Anda siap untuk menguji coba model klasifikasi ini!
...
1. Pastikan Anda sudah mengekstrak dan meletakkan folder `Dataset/` di *root* direktori proyek sesuai instruksi di atas.
...
4. Jalankan semua *cell* secara berurutan. Kode `!unzip` di dalam notebook Anda sudah diatur agar otomatis mengekstrak dataset tersebut!
...
6. Pindahkan file yang baru diunduh tersebut ke komputer lokal Anda (timpa file model lama yang ada di *root* proyek ini).
